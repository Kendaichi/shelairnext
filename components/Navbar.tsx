...
type MenuItem = { label: string; href: string; topLink?: true; icon?: React.ReactNode; highlighted?: boolean; external?: true };
...
const moreMenu: MenuItem[] = [
  ...
  { label: "Shelair Insights", href: "/resources", topLink: true, icon: <BookOpen   className="w-3.5 h-3.5 text-primary shrink-0" /> },
...
const Navbar = ({
  ...
  const serviceMenu: MenuItem[] = [
    ...
    { label: "Commercial Refrigeration", href: "https://acrorefrigeration.com.au", external: true as const },
  ];
  ...
  return (
    <header className="fixed top-0 left-0 right-0 z-50 bg-background/80 backdrop-blur-lg border-b border-border">
      <div className="container-narrow flex items-center justify-between h-16 md:h-20 px-6">
        ...
        <button onClick={() => setMobileOpen(!mobileOpen)} aria-label={mobileOpen ? "Close menu" : "Open menu"} className="lg:hidden p-2.5 -mr-1">
